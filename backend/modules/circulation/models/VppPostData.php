...
class VppPostData extends \yii\db\ActiveRecord
{
 ...
 /**
    ...
    private function getnextid(){
            ...
            $query = (new \yii\db\Query())->select(['id'])->from('vpp_post_data');
            $command = $query->createCommand();
            $data = $command->queryAll();
            $titles = '';
            foreach($data as $row) {
    ...
    public function agencylist(){
           ...
           $query = (new \yii\db\Query())->select('*')->from('agency')->where(['route_id' =>'4'])->orderBy('mail_state_id');
            $command = $query->createCommand();
