...
//    }
    ...
    public function getBill($month){
      ...
      public function getSinglebill($month,$id){
                $month=date('Ym');
             $query = (new \yii\db\Query())->select(['DATE_FORMAT(issue_date,"%Y%m") as month,agency.id as agencyid,agency.name as name,SUM(PJY) as pjy,SUM(org) as org,SUM(total_copies) as total_copies,SUM(total_price) as total_price,SUM(discounted_amt) as discounted_amt,SUM(final_total) as final_amt'])->from('agency_bill_book')->innerJoin('agency','agency.id=agency_bill_book.agency_id')->where('DATE_FORMAT(issue_date,"%Y-%m")="'.$month.'"')->where('agency.id="'.$id.'"')->groupBy('agency_id');
             $command = $query->createCommand();
             return $data = $command->queryAll();
            // $titles = '';
    ...
    public function getDetails($month,$agency_id){
         $query = (new \yii\db\Query())->select(['agency_id as id,discount as discount,price_per_piece as unitprice,DATE_FORMAT(issue_date,"%d-%b-%y") as date,agency.id as agencyid,agency.name as name,agency_bill_book.pjy as pjy,agency_bill_book.org as org,total_copies,total_price,discounted_amt,final_total'])->from('agency_bill_book')->innerJoin('agency','agency.id=agency_bill_book.agency_id')->where(['DATE_FORMAT(issue_date,"%Y-%m")'=>$month,'agency_id'=>$agency_id]);
             $command = $query->createCommand();
             return $data = $command->queryAll();
            // $titles = '';
    ...
    public function getPrintdetailsmonth($month,$agency_id){
         $query = (new \yii\db\Query())->select(['*,discount as discount,price_per_piece as unitprice,DATE_FORMAT(issue_date,"%M-%Y") as months,DATE_FORMAT(issue_date,"%d-%b-%y") as date,agency.id as agencyid,agency.name as name,agency_bill_book.pjy as pjy,agency_bill_book.org as org,total_copies,total_price,discounted_amt,final_total'])->from('agency_bill_book')->innerJoin('agency','agency.id=agency_bill_book.agency_id')->where(['DATE_FORMAT(issue_date,"%Y-%m")'=>$month,'agency_id'=>$agency_id]);
             $command = $query->createCommand();
             return $data = $command->queryAll();
            // $titles = '';
